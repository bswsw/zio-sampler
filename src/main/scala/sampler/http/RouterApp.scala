...
import zio.{Runtime, Scope, ULayer, ZIO, ZIOAppArgs, ZIOAppDefault, ZLayer}
...
object RouterApp extends ZIOAppDefault {
  ...
  private val router = Http.collectZIO[Request] {
    case Method.GET -> !! / "sample" / path =>
      for {
  ...
  private val server = Server.serve(app)
